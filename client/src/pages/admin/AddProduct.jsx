...
const AddProduct = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setProductName(''); setDescription(''); setCategory(''); setPrice(''); setOfferPrice('');
  ...
  return (
    <div className="py-10 bg-white min-h-screen">
      <form onSubmit={handleSubmit} className="md:p-10 p-4 space-y-6 max-w-lg mx-auto">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-3 mt-2">
            {Array(4).fill('').map((_, index) => (
              <label key={index} htmlFor={`image${index}`} className="cursor-pointer w-20 h-20 sm:w-24 sm:h-24">
                <input
                  accept="image/*"
                  type="file"
                  id={`image${index}`}
                ...
                />
                <img
                  className="w-full h-full object-cover border p-1 rounded"
        ...
        <div className="flex flex-col gap-1">
          ...
          <input
            ...
            className="outline-none w-full py-2.5 px-3 rounded border border-gray-500/40"
            value={productName}
        ...
        <div className="flex flex-col gap-1">
          ...
          <textarea
            ...
            rows={4}
            className="outline-none w-full py-2.5 px-3 rounded border border-gray-500/40 resize-none"
            placeholder="Type here"
            value={description}
        ...
        <button
          type="submit"
          className="w-full px-8 py-2.5 bg-indigo-500 text-white font-medium rounded hover:opacity-90 transition"
          disabled={loading}
        >
          {loading ? 'Adding...' : 'ADD'}
        </button>
